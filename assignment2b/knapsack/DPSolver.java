...
public class DPSolver {
	...
	private String createSolution2(int[][] mat,byte[][] mat2, int[] weights, int[] values) {
		int k = mat.length-1;
		int N = weights.length;
		int[] takes = new int[N];
	...
	public String solve2(int K, int[] values, int[] weights ) {
			...
			for (int k=0; k <= K; k++) {
				...
				if (i==0) {
					...
					if (k < weights[i] ) {
						...
						// we did take item
						if (values[i]+mat[k-weights[i]][0]>mat[k][0]) {
							mat2[k][i] = 1;
						} else {
							mat2[k][i] = 0;
						}
		...
		String result = createSolution2(mat,mat2,weights,values);
		return(result);
	...
	public static void main(String[] args) {
		System.out.println("running Solver.java");
